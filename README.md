...
| **Document Parsing** | Built-in `parse_document` tool extracts text from PDF, DOCX, XLSX, XLS, CSV, HTML, TXT, MD, JSON, XML. All native Rust, zero external services: PDF text via `pdf-extract`, DOCX/XML via a `quick-xml` streaming walk, Excel via `calamine`, CSV via `csv`. Scanned/image-only PDFs fall back to page-image rendering for vision models (see **PDF Support** above). Excel files are parsed into readable table format with sheet headers. Reading PPTX and legacy binary `.doc`/`.ppt` is out of scope by design |
