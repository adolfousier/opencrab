...
| **Document Generation** | Built-in `generate_document` tool creates XLSX (live Excel formulas), DOCX, and PDF natively in Rust with zero host dependencies, plus PPTX via python-pptx when present. Full styling per format: brand colors, page headers/footers with logos and page numbers, zebra tables, frozen headers, autofilters, number formats, PowerPoint brand templates. Generated files are delivered as downloadable attachments on Telegram/WhatsApp/Discord. See [Document Generation](#-document-generation) |
...
| **DOCX** | Native Rust (`docx-rs`) | Headings (real Word styles, navigation pane works), paragraphs, bullet/numbered lists (real numbering), tables | Accent-colored headings, page header/footer on every page, shaded table headers, zebra rows |
| **PDF** | Native Rust (`printpdf` + bundled DejaVu Sans) | Same block model as DOCX; A4 flow with word wrap, page breaks, content-sized table columns with header separators and row rules; real Unicode text (accents, Cyrillic, arrows, checkmarks) | Brand accent + text colors, H1 underline bar, page header with **logo image** (local PNG/JPEG), footer with exact `Page N of M`, zebra tables |
...
| `parse_document` | Extract text from PDF, DOCX, XLSX, XLS, CSV, HTML, TXT, MD, JSON, XML |
...
│   ├── tests/            # 4,668 tests (see TESTING.md)
...
# Run tests (4,668 tests across 425 test modules; 25 slower tests are
