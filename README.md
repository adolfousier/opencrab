...
| **Dynamic Brain System** | System brain assembled from workspace MD files — all editable live between turns. Files can declare a custom description via YAML frontmatter (`---` fences, `description: "…"` field) which replaces the hardcoded description in the context-file index. CRLF and missing-trailing-newline tolerant; capped at 200 characters with `…` truncation so a runaway value can't bloat the system prompt |
