...
| **Telegram Userbot (optional)** | Feature-gated, receive-only MTProto companion. Local QR/code/2FA login; allowlisted text is passively stored under `telegram-userbot` for explicit retrieval through `channel_search`. Empty `allowed_chats` is dry mode. It does not invoke the agent or send/edit/react as the user. |
...
| `telegram-userbot` | grammers | Optional receive-only Telegram MTProto capture |
...
| `telegram-userbot` | Receive-only Telegram MTProto capture (optional) |
