...
use crate::memory::embedding::embed_content;
use std::sync::Mutex;
...
async fn append_skips_unchanged_chunks() {
    ...
    embed_content(store, body_v1).expect("First embed should succeed");
    ...
    embed_content(store, &body_v2).expect("Second embed should succeed");
...
async fn identical_content_skips_reembed() {
    ...
    embed_content(store, body).expect("First embed should succeed");
    embed_content(store, body).expect("Second embed should succeed");
