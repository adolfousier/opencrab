...
impl BenchmarkResult {
    fn print_table(results: &[BenchmarkResult]) {
        println!("\n{:<20} {:>12} {:>12} {:>12} {:>12}", "URL", "Time (ms)", "Bytes", "Calls", "Tokens");
        println!("{}", "-".repeat(80));
        for r in results {
            println!(
                "{:<20} {:>12} {:>12} {:>12} {:>12}",
...
async fn web_scrape_benchmark_comparison() {
    ...
    for (label, url) in BENCHMARK_URLS {
        ...
        println!("  http_request: {}ms, {} bytes", old.time_ms, old.output_bytes);
        old_results.push(old);
        
        // New approach
        let new = benchmark_web_scrape(label, url).await;
        println!("  web_scrape:   {}ms, {} bytes", new.time_ms, new.output_bytes);
    ...
    println!("\n━━━ IMPROVEMENT SUMMARY ━━━");
    println!("{:<20} {:>15} {:>15} {:>15}", "URL", "Bytes Reduction", "Token Reduction", "Calls Saved");
    println!("{}", "-".repeat(80));
    ...
    let avg_old_time = old_results.iter().map(|r| r.time_ms).sum::<u128>() / old_results.len() as u128;
    let avg_new_time = new_results.iter().map(|r| r.time_ms).sum::<u128>() / new_results.len() as u128;
    ...
    println!("   - Average time: {:.0}ms (old) vs {:.0}ms (new)", 
