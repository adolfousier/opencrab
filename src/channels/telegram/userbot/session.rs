...
impl FileSession {
    ...
    /// Persist only if mutated since the last save. Cheap no-op otherwise —
...
mod tests {
    ...
    async fn round_trips_auth_key_peers_and_state() {
        ...
        let dc = DcOption {
            id: 2,
            ipv4: "127.0.0.1:443".parse().expect("addr"),
            ipv6: "[::1]:443".parse().expect("addr"),
            auth_key: Some(key),
        };
        ...
        session
            ...
            .await
            .expect("set state");
        session.save_if_dirty().expect("dirty save");

        let reloaded = FileSession::load(&path).expect("reload");
        let dc2 = reloaded.dc_option(2).expect("dc read").expect("dc present");
        assert_eq!(dc2.auth_key, Some(key), "auth key must survive");
        assert!(
            reloaded
        ...
        let st = reloaded.updates_state().await.expect("state read");
        assert_eq!((st.pts, st.date, st.seq), (111, 222, 3));

        // Clean reload parses as a no-op dirty save.
        reloaded.save_if_dirty().expect("clean save is no-op");
        assert_eq!(
