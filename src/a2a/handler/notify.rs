...
pub async fn handle_session_notify(
    ...
    let (outcome, detail) = match deliver_to_session(session_id, msg, false) {
        ...
        // idle or resend with interrupt.
        Delivery::RefusedInFlight => (
            "refused_in_flight",
            format!(
                "refused: session {session_id} is mid-turn and interrupt was not set — \
        ...
        // Fork #17: interrupt does NOT override this gate — a replaced
        // session must never be woken into its successor's conversation.
        // Name the occupant so the sender can redirect.
        Delivery::RefusedChannelOccupied { occupant } => (
            "refused_channel_occupied",
            format!(
                "refused: session {session_id} no longer owns its channel — the chat/topic \
                 it was bound to is now occupied by session {occupant}. Waking it would post \
                 into {occupant}'s conversation; notify {occupant} instead if the message \
