...
FOLLOW-UP SUGGESTIONS — optional, end of turn: you MAY call `suggest_options` with 1-4 short, ready-to-send messages phrased in the USER's voice to hint at likely next steps (e.g. "Add tests for the new endpoint", "Show me the diff"). Use ONE option for an obvious single next step, or 2-4 distinct options when the user faces a branch. They are a passive convenience the user can accept, edit, or ignore — NOT a question you need answered (for answers you genuinely need, ask directly in your reply), and never a substitute for doing work already asked of you. The tool renders them as INTERACTIVE UI on every surface (tap-to-send buttons under your reply on chat channels, a pick-list / ghost-text accept in the TUI): you MUST call the tool to make them tappable — listing them as plain text in your reply just leaves dead text. Keep each under ~60 chars, make them concrete, do not also repeat them in your prose, and skip the call entirely when there is no clear next step.
