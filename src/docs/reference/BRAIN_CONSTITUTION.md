...
> **Last updated:** 2026-06-22 (v0.3.47) — gaps #5/#6/#8/#9/#10 resolved, #1/#11/#13 addressed; JIT tool activation landed (commit `56b8b057`); per-project brain overlay (`87621927`); system brain rebuild on change (`ee367d2d`, #213); AGENTS.md confirmed LAST in the prompt; ownership map single-sourced to the preamble + RSI taxonomy.
...
13. **`lazy_tools` creates permanent gap between brain files and tool discovery.** When `lazy_tools = true` (default), EXTENDED tools are only added to the system prompt after `tool_search` discovers them. But the executor resolves tools from the full registry — so the model can call any tool by name, even if it is not in the active set. This creates a permanent mismatch: brain files reference tools by name, the model tries to call them, the executor finds them in the registry and runs them, but the model has no parameter schema (not in system prompt). The model guesses parameters → often fails. The tool is NOT added to `active_extended` → next call also guesses. RSI records the failure as "tool X is unreliable" instead of "tool X was never discovered." **Verified ACTUAL (2026-06-22):** `telegram_send` — 61.7% failure rate (107 calls), `tg_mtproto` — 57.7% failure rate. Root cause: only `tool_search` promotes tools into `active_extended`. The executor can run any tool, but never promotes. The tool remains invisible to the model permanently. **Related:** [Issue #214](https://github.com/adolfousier/opencrabs/issues/214). **✅ RESOLVED (2026-06-22, commit `56b8b057`, v0.3.47):** JIT tool activation now promotes extended tools into `active_extended` on-demand when called by name. Additionally, extended tool guidance was moved to the preamble + RSI (`56df0352`), and pre-execution misses no longer penalize RSI tool success rates (`5d0b4b47`).

   **What was suggested vs what shipped:** The suggested JIT promotion approach matched the implementation: when the executor receives a call for an undiscovered tool, it promotes it before executing. The complementary RSI fix (separating discovery failures from real tool errors) also shipped — pre-execution misses are now excluded from success rate calculations.
...
| `AGENTS.md` | Core brain file | Always (LAST, after SOUL.md) | ~13KB |
| `CODE.md` | Contextual brain file | On demand | ~6KB |
| `TOOLS.md` | Contextual brain file | On demand | ~32KB |
...
## Appendix C: v0.3.47 Sync Changelog (2026-06-22)

37 commits reviewed from `origin/main` since v0.3.46 (2026-06-21). 52 files changed, +2665/-289 lines, 4103 tests.

### Brain-relevant commits

| SHA | Category | Summary |
|-----|----------|---------|
| `56b8b057` | Tools | JIT tool activation — extended tools promoted to `active_extended` on-demand when called by name (gap #13 resolved) |
| `56df0352` | Prompt composition | Extended tool guidance moved to BRAIN_PREAMBLE + RSI taxonomy |
| `5d0b4b47` | RSI | Pre-execution tool misses excluded from RSI success rate calculations |
| `e3f20a64` | Prompt composition | PROACTIVE TOOL DISCOVERY + PROJECT DIRECTIVES directives in BRAIN_PREAMBLE (gaps #9/#10/#11 resolved) |
| `87621927` | Brain files | Per-project brain overlay — project-specific brain files layer on top of global ones |
| `ee367d2d` | Brain files | System brain rebuild on brain file change (fixes #213) |
| `c8ddb521` | Security | Confidential file protection — sensitive files excluded from tool output |
| `1adc8b17` | Security | Owner impersonation detection — agent cannot masquerade as the user |
| `217a431e` | Channels | Forum topic session isolation (fixes #215) |

### Non-brain commits (noted for completeness)

| SHA | Category | Summary |
|-----|----------|---------|
| Various | CI/Tests | 4103 tests passing, test infrastructure improvements |
| Various | Channels | Discord, Slack, WhatsApp, Telegram refinements |
| Various | Browser | CDP automation improvements |
| Various | Tools | Various tool fixes and improvements |

### Known Gaps verified (2026-06-22)

| Gap | Status | Evidence |
|-----|--------|----------|
| #1 (Project-repo discovery) | **🟡 ADDRESSED** | Per-project brain overlay (`87621927`) goes beyond directive-level fix |
| #11 (Project persistence) | **✅ ADDRESSED** | System brain rebuild on change (`ee367d2d`, #213) reinforces directive-level fix |
| #12 (Rule activation modes) | **🔴 STILL ACTUAL** | Binary always/contextual only (recommended: defer) |
| #13 (lazy_tools JIT gap) | **✅ RESOLVED** | JIT tool activation (`56b8b057`) — executor promotes before executing |
